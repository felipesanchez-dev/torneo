...
const GoalModal: React.FC<GoalModalProps> = ({ isVisible, onClose, onAddGoal, currentMinute, teams, matchId = "" }) => {
  ...
  const updateMatchData = async (team: Team, scorer: Player, assistant: Player | null) => {
    ...
    try {
      ...
      const existingGoals = Array.isArray(matchData.goals_data) ? matchData.goals_data : []
      ...
      const team1Goals = Number.parseInt(
        matchData.results && matchData.results[team1Id] && matchData.results[team1Id].goals
          ? matchData.results[team1Id].goals
          : "0",
      )
      const team2Goals = Number.parseInt(
        matchData.results && matchData.results[team2Id] && matchData.results[team2Id].goals
      ...
      if (
        ...
        const teamGoalsMatch = matchData.performance[team.id]["0"].goals.match(/^(\d+)(?:\s*$$(.*?)$$)?$/)
        if (teamGoalsMatch) {
          currentTeamGoals = Number.parseInt(teamGoalsMatch[1])
          // Extract existing minutes if they exist
          if (teamGoalsMatch[2]) {
            existingTeamGoalMinutes = teamGoalsMatch[2].split(", ").map((m) => m.replace("'", ""))
          }
        } else {
          // If no match, just try to parse as a number
          currentTeamGoals = Number.parseInt(matchData.performance[team.id]["0"].goals) || 0
        }
      }
      ...
      if (assistant) {
        ...
        if (
          ...
          const teamAssistsMatch = matchData.performance[team.id]["0"].assists.match(/^(\d+)(?:\s*$$(.*?)$$)?$/)
          if (teamAssistsMatch) {
            currentTeamAssists = Number.parseInt(teamAssistsMatch[1])
            // Extract existing minutes if they exist
            if (teamAssistsMatch[2]) {
              existingTeamAssistMinutes = teamAssistsMatch[2].split(", ").map((m) => m.replace("'", ""))
            }
          } else {
            // If no match, just try to parse as a number
            currentTeamAssists = Number.parseInt(matchData.performance[team.id]["0"].assists) || 0
          }
        }
      ...
      if (scorer?.id) {
        ...
        const existingPlayerData =
          matchData.performance && matchData.performance[team.id] && matchData.performance[team.id][scorer.id]
        ...
        if (existingPlayerData.goals) {
          // Check if the goals field already has minute information
          const goalsMatch = existingPlayerData.goals.match(/^(\d+)(?:\s*$$(.*?)$$)?$/)
          if (goalsMatch) {
            currentGoals = Number.parseInt(goalsMatch[1])
            // Extract existing minutes if they exist
            if (goalsMatch[2]) {
              existingGoalMinutes = goalsMatch[2].split(", ").map((m) => m.replace("'", ""))
            }
          } else {
            // If no match, just try to parse as a number
        ...
        existingPlayerData.goals = `${currentGoals + 1} (${existingGoalMinutes.join("', ")}')`
      ...
      if (assistant?.id) {
        // Get existing player data if available
        const existingPlayerData =
          matchData.performance && matchData.performance[team.id] && matchData.performance[team.id][assistant.id]
        ...
        if (existingPlayerData.assists) {
          // Check if the assists field already has minute information
          const assistsMatch = existingPlayerData.assists.match(/^(\d+)(?:\s*$$(.*?)$$)?$/)
          if (assistsMatch) {
            currentAssists = Number.parseInt(assistsMatch[1])
            // Extract existing minutes if they exist
            if (assistsMatch[2]) {
              existingAssistMinutes = assistsMatch[2].split(", ").map((m) => m.replace("'", ""))
            }
          } else {
            // If no match, just try to parse as a number
        ...
        existingPlayerData.assists = `${currentAssists + 1} (${existingAssistMinutes.join("', ")}')`
