...
export default function GameScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"eventos" | "estadisticas">("eventos")
  const [refreshing, setRefreshing] = useState(false)

  const { team1, team2, matchId } = useLocalSearchParams()

  const parsedTeam1 = typeof team1 === "string" ? JSON.parse(decodeURIComponent(team1)) : null
  const parsedTeam2 = typeof team2 === "string" ? JSON.parse(decodeURIComponent(team2)) : null
...
const handleEditEvent = (event: MatchEvent) => {
  ...
  const extractMinutesFromString = (formattedString: string): number[] => {
    ...
    if (match && match[1]) {
      ...
      return match[1].split(", ").map((m) => Number.parseInt(m.replace("'", "")))
    }
    return []
  }
  ...
  const fetchMatchData = async () => {
    try {
      ...
      const response = await fetch(`https://luxoradevs.com/jp/wp-json/sportspress/v2/events/${parsedMatchId}`)
      ...
      setMatchData({
        ...
        yellowCards: {
          local: Number.parseInt(data.performance[localTeamId]?.[0]?.yellowcards || 0),
          visitante: Number.parseInt(data.performance[visitorTeamId]?.[0]?.yellowcards || 0),
        },
        redCards: {
          local: Number.parseInt(data.performance[localTeamId]?.[0]?.redcards || 0),
          visitante: Number.parseInt(data.performance[visitorTeamId]?.[0]?.redcards || 0),
        },
        blueCards: {
          local: Number.parseInt(data.performance[localTeamId]?.[0]?.azul || 0),
          visitante: Number.parseInt(data.performance[visitorTeamId]?.[0]?.azul || 0),
        },
      })
      ...
      if (data.performance && data.performance[localTeamId]) {
        Object.entries(data.performance[localTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId === "0") return // Skip metadata

          const hasGoals = playerData.goals && playerData.goals.trim() !== ""
          const hasAssists = playerData.assists && Number.parseInt(playerData.assists) > 0
          const hasYellowCards = playerData.yellowcards && playerData.yellowcards.trim() !== ""
          const hasRedCards = playerData.redcards && playerData.redcards.trim() !== ""
          const hasBlueCards = playerData.azul && Number.parseInt(playerData.azul) > 0

          if (hasGoals || hasAssists || hasYellowCards || hasRedCards || hasBlueCards) {
      ...
      if (data.performance && data.performance[visitorTeamId]) {
        Object.entries(data.performance[visitorTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId === "0") return // Skip metadata
      ...
      for (const playerId of activePlayerIds) {
        try {
          const playerResponse = await fetch(`https://luxoradevs.com/jp/wp-json/sportspress/v2/players/${playerId}`)
      ...
      if (data.performance && data.performance[localTeamId]) {
        Object.entries(data.performance[localTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId !== "0" && playerData.goals && playerData.goals.trim() !== "") {
            const playerName = playerMapping[Number.parseInt(playerId)] || `Jugador ${playerId}`

            // Extract goal count and minutes
            const goalsMatch = playerData.goals.match(/^(\d+)(?:\s*\((.*?)\))?$/);
            if (goalsMatch) {
              ...
              const minutes = minutesString
                ? minutesString.split(", ").map((m: string) => Number.parseInt(m.replace("'", "")))
                : [];
      ...
      if (data.performance && data.performance[localTeamId]) {
        Object.entries(data.performance[localTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId !== "0" && playerData.assists && Number.parseInt(playerData.assists) > 0) {
            const playerName = playerMapping[Number.parseInt(playerId)] || `Jugador ${playerId}`
            ...
            for (let i = 0; i < Number.parseInt(playerData.assists); i++) {
              ...
              const goalWithoutAssist = newEvents.find(
                (event) => event.type === "gol" && !event.assistant && event.playerId !== Number.parseInt(playerId),
              )
      ...
      if (data.performance && data.performance[localTeamId]) {
        Object.entries(data.performance[localTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId !== "0" && playerData.yellowcards && playerData.yellowcards.trim() !== "") {
            const playerName = playerMapping[Number.parseInt(playerId)] || `Jugador ${playerId}`

            // Extract card count and minutes
            const cardsMatch = playerData.yellowcards.match(/^(\d+)(?:\s*\((.*?)\))?$/);
            if (cardsMatch) {
      ...
      if (data.performance && data.performance[localTeamId]) {
        Object.entries(data.performance[localTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId !== "0" && playerData.redcards && playerData.redcards.trim() !== "") {
            const playerName = playerMapping[Number.parseInt(playerId)] || `Jugador ${playerId}`

            // Extract card count and minutes
            const cardsMatch = playerData.redcards.match(/^(\d+)(?:\s*\((.*?)\))?$/);
            if (cardsMatch) {
              const cardCount = Number.parseInt(cardsMatch[1]) || 0
              const minutesString = cardsMatch[2] || ""
      ...
      if (data.performance && data.performance[localTeamId]) {
        Object.entries(data.performance[localTeamId]).forEach(([playerId, playerData]: [string, any]) => {
          if (playerId !== "0" && playerData.azul && Number.parseInt(playerData.azul) > 0) {
            const playerName = playerMapping[Number.parseInt(playerId)] || `Jugador ${playerId}`

            const cardsMatch = playerData.azul.match(/^(\d+)(?:\s*\((.*?)\))?$/);
            if (cardsMatch) {
              const cardCount = Number.parseInt(cardsMatch[1]) || 0
              const minutesString = cardsMatch[2] || ""
  ...
  const handleAddGoal = (team: Team, player: Player, assistant: Player | null, minute: number) => {
  ...
  const handleAddYellowCard = (team: Team, player: Player, cardMinute: number) => {
  ...
  const handleAddBlueCard = (team: Team, player: Player, cardMinute: number) => {
  ...
  const handleDeleteEvent = (eventId: any, type: string, cardType: any, team: string, event: any) => {
    ...
    if (type === "gol") {
      ...
      const currentScore = isLocal ? matchData.localScore : matchData.visitorScore;
    ...
    if (type === "tarjeta") {
      const isLocal = team === "local";
      const playerId = event.playerId?.toString() || "0";
      const teamKey = isLocal ? parsedTeam1?.id?.toString() || "" : parsedTeam2?.id?.toString() || "";
    
      if (cardType === "amarilla") {
        const currentScore = isLocal ? matchData.yellowCards.local : matchData.yellowCards.visitante;
      ...
      if (cardType === "roja") {
        const currentScore = isLocal ? matchData.redCards.local : matchData.redCards.visitante;
        const newScore = Math.max(currentScore - 1, 0);
      ...
      if (cardType === "azul") {
        const currentScore = isLocal ? matchData.blueCards.local : matchData.blueCards.visitante;
        const newScore = Math.max(currentScore - 1, 0);
  ...
  const updateResponse = async (body: any) => {
    setLoading(true);
    try {
      const response = await fetch(`https://luxoradevs.com/jp/wp-json/sportspress/v2/events/${matchId}`, {
        method: "POST",
        headers: {
      ...
      Alert.alert("Error", "No se pudo actualizar el partido.", [{ text: "OK" }])
    } finally {
      setLoading(false)
      return false
    }
    ...
    return (
      ...
      >
        <SafeAreaView style={[styles.safeArea, styles.loadingContainer]}>
          ...
          <Text style={[styles.loadingText, { color: "#FFFFFF" }]}>Cargando datos del partido...</Text>
        </SafeAreaView>
      </LinearGradient>
    )
  }
    ...
    return (
      ...
      >
        <SafeAreaView style={[styles.safeArea, styles.errorContainer]}>
          ...
          <Text style={[styles.errorText, { color: "#FFFFFF" }]}>Error al cargar los datos</Text>
          <Text style={[styles.errorDetail, { color: "#FFFFFF" }]}>{error}</Text>
        </SafeAreaView>
      </LinearGradient>
    )
  }
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          >
              ...
              <View style={styles.actionButtonsContainer}>
                ...
                >
                  <TouchableOpacity style={styles.actionButton} onPress={() => setModalVisible(true)}>
                    <Ionicons name="trophy" size={18} color={theme.textPrimary} />
                  ...
                  >
                    <Ionicons name="shield-outline" size={18} color={theme.textPrimary} />
                  ...
                  >
                    <Ionicons name="shield-outline" size={18} color={theme.accent1} />
                    <Text style={styles.actionButtonText}>Roja</Text>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.actionButton, { backgroundColor: "#CCE5FF" }]}
                    onPress={() => setBlueCardModalVisible(true)}
                  >
                    <Ionicons name="shield-outline" size={18} color={theme.accent3} />
            ...
            <View style={styles.tabContainer}>
              ...
              >
                <Text style={[styles.tabButtonText, activeTab === "eventos" && styles.activeTabButtonText]}>
                  Eventos
                </Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.tabButton, activeTab === "estadisticas" && styles.activeTabButton]}
                onPress={() => setActiveTab("estadisticas")}
              >
                <Text style={[styles.tabButtonText, activeTab === "estadisticas" && styles.activeTabButtonText]}>
                  Estadísticas
                </Text>
              </TouchableOpacity>
            </View>
            ...
            {activeTab === "eventos" ? (
              <View style={[styles.contentCard, events.length === 0 && styles.emptyContentCard]}>
                <Text style={styles.contentTitle}>Cronología del Partido</Text>

                {eventsLoading ? (
                  <View style={styles.eventsLoadingContainer}>
                    <ActivityIndicator size="large" color={theme.primary} />
                    <Text style={styles.eventsLoadingText}>Cargando eventos...</Text>
                  </View>
                ) : events.length === 0 ? (
                  <View style={styles.emptyStateContainer}>
                    <Text style={styles.noEventsText}>No hay eventos registrados para este partido</Text>
                  </View>
                ) : (
                  events.map((event) => (
                    <View key={event.id} style={styles.eventItem}>
                      <View style={styles.eventIconContainer}>
                        {event.type === "gol" ? (
                          <Ionicons name="trophy-outline" size={20} color={theme.textPrimary} />
                        ) : (
                          <Ionicons
                            name="shield-outline"
                            size={20}
                      ...
                      <View style={styles.eventContent}>
                        <Text style={styles.eventTitle}>
                          {event.type === "gol" ? "Gol" : `Tarjeta ${event.cardType}`}
                        </Text>
                        <Text style={styles.eventPlayer}>
                          {event.player} ({event.team === "local" ? matchData.localTeam : matchData.visitorTeam})
                          {event.type === "gol" && event.assistant && (
                            <Text style={styles.eventAssistant}> - Asistencia: {event.assistant}</Text>
                          )}
                        </Text>
                      </View>
              ...
              <View style={styles.contentCard}>
                ...
                <View style={styles.statSection}>
                  ...
                  <View style={styles.statRow}>
                    <View style={styles.statTeam}>
                      <Text style={styles.statTeamName} numberOfLines={2} ellipsizeMode="tail">
                        {matchData.localTeam}
                      </Text>
                    ...
                    <View style={styles.statTeam}>
                      ...
                      <Text style={styles.statValue}>{stats.goals.visitante}</Text>
                    </View>
                  </View>
                </View>
                ...
                <View style={styles.statSection}>
                  ...
                  <View style={styles.statRow}>
                    <View style={styles.statTeam}>
                      <Text style={styles.statTeamName}>{matchData.localTeam}</Text>
                      <View style={styles.cardContainer}>
                        <View style={[styles.cardCircle, { backgroundColor: theme.primary }]}>
                          <Text style={styles.cardCount}>{stats.cards.local.amarilla}</Text>
                        </View>
                        <View style={[styles.cardCircle, { backgroundColor: theme.accent1 }]}>
                          <Text style={styles.cardCount}>{stats.cards.local.roja}</Text>
                        </View>
                        <View style={[styles.cardCircle, { backgroundColor: theme.accent3 }]}>
                          <Text style={styles.cardCount}>{stats.cards.local.azul}</Text>
                        </View>
                      </View>
                    </View>

                    <View style={styles.statTeam}>
                      <Text style={styles.statTeamName}>{matchData.visitorTeam}</Text>
                      <View style={styles.cardContainer}>
                        <View style={[styles.cardCircle, { backgroundColor: theme.primary }]}>
                          <Text style={styles.cardCount}>{stats.cards.visitante.amarilla}</Text>
                        </View>
                        <View style={[styles.cardCircle, { backgroundColor: theme.accent1 }]}>
                          <Text style={styles.cardCount}>{stats.cards.visitante.roja}</Text>
                        </View>
                        <View style={[styles.cardCircle, { backgroundColor: theme.accent3 }]}>
                          <Text style={styles.cardCount}>{stats.cards.visitante.azul}</Text>
                        </View>
                      </View>
                    </View>
                  </View>
                ...
                <View style={styles.statSection}>
                  ...
                  <View style={styles.infoRow}>
                    ...
                    <Text style={styles.infoValue}>{matchData.league || "No especificada"}</Text>
                  </View>
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Temporada:</Text>
                    <Text style={styles.infoValue}>{matchData.season || "No especificada"}</Text>
                  </View>
                  <View style={styles.infoRow}>
